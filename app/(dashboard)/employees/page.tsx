...
import { PageHeader, Card, StatusPill, Button, Avatar, SearchInput, Tabs, EmptyState } from "@/components/ui";
import { mockEmployees, type Employee } from "@/lib/mock-data";
import { Pencil, Mail, Phone, MapPin, Calendar, Briefcase, X } from "lucide-react";
...
export default function EmployeesPage() {
  ...
  return (
    <div>
      <PageHeader title="Employees" subtitle="Manage your organization's employee records." />
                  ...
                  {filtered.map((e) => (
                    <tr
                      ...
                      className={`cursor-pointer transition-colors ${
                        selected?.id === e.id ? "bg-[#e6f5f3]" : "hover:bg-slate-50"
                      }`}
                    >
          ...
          <div className="lg:col-span-1">
            <Card noPadding className="overflow-hidden">
              ...
              <div className="relative p-6 bg-gradient-to-br from-[#006b5f] to-[#014239] text-white">
                ...
                <Avatar name={selected.name} size="xl" className="!bg-white/20 mb-3" />
                <h3 className="text-lg font-bold font-heading">{selected.name}</h3>
                <p className="text-sm text-white/80">{selected.position}</p>
                <p className="text-xs text-white/60 mt-0.5">{selected.department}</p>
              ...
              <div className="p-6">
                {activeTab === "profile" && (
                  <div className="space-y-5">
                    <div className="grid grid-cols-1 gap-4">
                      <ContactField icon={Mail} label="Email" value={selected.email} />
                      <ContactField icon={Phone} label="Phone" value={selected.phone} />
                      <ContactField icon={MapPin} label="Location" value={selected.location} />
                      <ContactField icon={Calendar} label="Join Date" value={new Date(selected.joinDate).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })} />
                      <ContactField icon={Briefcase} label="Department" value={selected.department} />
                    </div>
                    <Button variant="secondary" className="w-full" icon={<Pencil className="h-4 w-4" />} onClick={() => console.log("edit")}>
                      Edit Profile
                    </Button>
                  </div>
                )}
                ...
                {activeTab !== "profile" && (
                  <EmptyState
                    icon={Briefcase}
                    title={detailTabs.find((t) => t.id === activeTab)?.label ?? ""}
