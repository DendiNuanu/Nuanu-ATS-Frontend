...
export default function AIScoringPage() {
  ...
  const candidates = mockCandidates.filter((c) => c.stage === "New" || c.stage === "Screening");
          ...
          <Card key={c.id} className="!p-5">
            <div className="flex flex-col sm:flex-row sm:items-center gap-5">
              ...
              <div className="hidden lg:block flex-shrink-0 w-56 space-y-2">
                {[
                  { label: "Skills", value: Math.min(100, c.aiMatch + 3) },
                  { label: "Experience", value: Math.min(100, c.aiMatch - 4) },
                  { label: "Education", value: Math.min(100, c.aiMatch - 8) },
                ].map((s) => (
                  <div key={s.label}>
