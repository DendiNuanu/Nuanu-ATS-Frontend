...
export default function CandidateDetailPage({
  ...
  const referAsValues = candidate.referAsSlots?.filter(Boolean) ??
  ...
  return (
    <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-5">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 mt-4 text-sm text-slate-500">
              ...
              <span className="inline-flex items-center gap-1.5">
                ...
                Applied {new Date(candidate.appliedDate).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => router.push(`/candidates/${id}/compose`)}
            >
              Message
            </Button>
            <Button
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card title="Personal Information">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
                ...
                <div>
                  <p className="text-xs font-medium text-slate-500 mb-1.5">Applied For</p>
                ...
                <div>
                  <p className="text-xs font-medium text-slate-500 mb-1.5">Refer As</p>
                ...
                <div>
                  <p className="text-xs font-medium text-slate-500 mb-1.5">Current Stage</p>
                  <StatusPill status={candidate.stage} />
                </div>
                <Field label="Expected Monthly Salary" value={candidate.expectedSalary ?? "-"} />
              </div>
            </Card>
          ...
          <div className="space-y-6">
            <Card title="AI Match Analysis">
              <div className="flex flex-col items-center text-center py-4">
                ...
                <p className="text-sm text-slate-500 mt-4">
                  Strong alignment with the {candidate.position} role based on skills, experience, and seniority.
                </p>
              </div>
            ...
            <Card title="Quick Actions">
              <div className="flex flex-col gap-2">
                <Button variant="secondary" className="w-full justify-start" icon={<Sparkles className="h-4 w-4" />} onClick={() => console.log("rescore")}>
                  Re-run AI Scoring
                </Button>
                <Button variant="secondary" className="w-full justify-start" icon={<Mail className="h-4 w-4" />} onClick={() => console.log("email")}>
                  Send Email
                </Button>
              </div>
            </Card>
      ...
      {activeTab === "interviews" && (
        <InterviewResultsTab candidateName={candidate.name} candidateId={candidate.id} />
      )}

      {activeTab === "references" && (
        <ReferenceChecksTab candidateName={candidate.name} candidatePosition={candidate.position} />
      )}
        ...
        <Card>
          <div className="text-center py-16">
            ...
            <p className="text-sm text-slate-500 max-w-sm mx-auto">
              This tab content will be populated in the full application. This is a placeholder for the prototype.
            </p>
          </div>
        </Card>
      )}
